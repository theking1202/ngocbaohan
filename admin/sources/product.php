...

{
    ...
    if ($data) {
        ...
        $data['gia'] = (isset($data['gia']) && $data['gia'] != '') ? intval(str_replace(",", "", $data['gia'])) : 0;
    ...
    if ($id) {
        ...
        if ($d->update('size', $data)) {
            ...
            // Update product prices based on size price and product weight

            if (isset($data['gia']) && !empty($data['gia'])) {
                ...
                $sql = "UPDATE #_product SET regular_price = (trong_luong * ?) WHERE id_size = ?";

                $result = $d->rawQuery($sql, array($gia, $size_id));
        ...
        if ($d->insert('size', $data)) {
            ...
            $size_id = $d->getLastInsertId();

            
            // Update product prices based on size price and product weight for new size
            ...
            if (isset($data['gia']) && !empty($data['gia'])) {
                ...
                // $sql = "UPDATE #_product SET regular_price = (khoi_luong * ?) WHERE id_size = ?";
