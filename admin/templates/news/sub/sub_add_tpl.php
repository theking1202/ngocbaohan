    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active"><?= $config['news'][$type]['title_main_sub'] ?></li>
            </ol>
        </div>
    </div>
</section>
                                        ...
                                        <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if (isset($config['news'][$type]['desc_sub']) && $config['news'][$type]['desc_sub'] == true) { ?>
                                                <div class="form-group">
                                                    ...
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['news'][$type]['desc_cke_sub']) && $config['news'][$type]['desc_cke_sub'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[desc<?= $k ?>]" id="desc<?= $k ?>" rows="5" placeholder="<?=mota?> (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('desc' . $k)) ?: $func->decodeHtmlChars(@$item['desc' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
                                            <?php if (isset($config['news'][$type]['content_sub']) && $config['news'][$type]['content_sub'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="content<?= $k ?>"><?=noidung?> (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['news'][$type]['content_cke_sub']) && $config['news'][$type]['content_cke_sub'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[content<?= $k ?>]" id="content<?= $k ?>" rows="5" placeholder="<?=noidung?> (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('content' . $k)) ?: $func->decodeHtmlChars(@$item['content' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
