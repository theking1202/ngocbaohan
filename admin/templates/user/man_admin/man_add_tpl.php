            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="custom-select text-sm" name="data[city]" id="city">
                            <option value="">Chọn tỉnh/thành phố</option>
                            <?php
                            // Load city data directly from database table
                            global $d;
                            $cities = $d->rawQuery("select id, name from table_city where find_in_set('hienthi',status) order by numb,id desc");
                            $selectedCity = (!empty($flash->has('city'))) ? $flash->get('city') : @$item['city'];
                            foreach ($cities as $city) {
                                echo '<option value="' . $city['id'] . '"' . (($selectedCity == $city['id']) ? ' selected' : '') . '>' . $city['name'] . '</option>';
                            }
                            ?>
                        </select>
                    </div>
