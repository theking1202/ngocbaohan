...
<section class="content">
        ...
        <form method="post" action="<?= $linkUploadImg ?>" enctype="multipart/form-data">
            <div class="card card-primary card-outline text-sm mb-0">
                <div class="card-header">
                    <h3 class="card-title"><?=hinhanhimport?></h3>
                </div>
                <div class="card-body">
                    <label class="text-danger"><?=cach1?>:</label>
                    <div class="alert my-alert alert-success mb-2" role="alert"><?=bancothenhaphinhbangcachcopyduongdanhinhonlinevaotaptinexcel?></div>
                    <div class="alert my-alert alert-info" role="alert">
                        <p class="mb-0"><?=vidu?>: <strong>https://tenmien.com/upload/ten-hinh-anh.png</strong></p>
                        <p class="mb-0 text-danger">=> <?=copyduongdannhuvidutrenvaotaptinexceldeimporthinhanh?></p>
                    </div>
                    <label class="text-danger"><?=cach2?>:</label>
                    <div class="form-group mb-0">
                        <label for="filer-import" class="label-filer-gallery mb-3">Album: (<?= $config['import']['img_type'] ?>)</label>
            ...
            <div class="card-footer text-sm">
                <button type="submit" class="btn btn-sm bg-gradient-primary" name="uploadImg"><i class="far fa-save mr-2"></i><?=luu?></button>
            </div>
        </form>
        <?php if (count($items)) { ?>
            <div class="card card-primary card-outline text-sm">
                <div class="card-header">
                    <h3 class="card-title"><?=danhsachhinhanhimport?></h3>
                </div>
                            ...
                            <?php for ($i = 0; $i < count($items); $i++) { ?>
                                <tr>
                                    ...
                                    <td class="align-middle text-center text-md text-nowrap">
                                        <a class="text-primary mr-2" href="<?= $linkEditImg ?>&id=<?= $items[$i]['id'] ?>" title="<?=chinhsua?>"><i class="fas fa-edit"></i></a>
            ...
            <?php } ?>
            <div class="card-footer text-sm">
                <a class="btn btn-sm bg-gradient-danger text-white" id="delete-all" data-url="<?= $linkDeleteImg ?>" title="<?=xoatatca?>"><i class="far fa-trash-alt mr-2"></i><?=xoatatca?></a>
            </div>
        <?php } ?>
    <?php } ?>
    <form method="post" action="<?= $linkUploadExcel ?>" enctype="multipart/form-data">
        <div class="card card-primary card-outline text-sm mb-0">
            <div class="card-header">
                <h3 class="card-title"><?=importdanhsachdulieu?></h3>
            </div>
            <div class="card-body">
                <div class="form-group">
                    <label class="d-inline-block align-middle mb-1 mr-2"><?=uploadtaptin?>:</label>
                    <strong class="d-block mt-2 mb-2 text-sm"><?=loai?> : .xls|.xlsx (Ms.Excel 2003 - 2007)</strong>
                    <div class="custom-file my-custom-file">
                        <input type="file" class="custom-file-input" name="file-excel" id="file-excel">
                        <label class="custom-file-label" for="file-excel"><?=chonfile?></label>
                    </div>
                </div>
            </div>
        </div>
