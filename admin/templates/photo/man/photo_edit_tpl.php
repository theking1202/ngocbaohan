    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active"><?=capnhat?> <?= $config['photo']['man_photo'][$type]['title_main_photo'] ?></li>
            </ol>
        </div>
    </div>
</section>
    ...
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <div class="card card-primary card-outline text-sm">
            <div class="card-header">
                <h3 class="card-title"><?=chitiet?></h3>
            </div>
            <div class="card-body">
                ...
                <?php if (isset($config['photo']['man_photo'][$type]['link2_photo']) && $config['photo']['man_photo'][$type]['link2_photo'] == true) { ?>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control text-sm" name="data[link2]" id="link2<?= $i ?>" placeholder="Link" value="<?= (!empty($flash->has('link2'))) ? $flash->get('link2') : @$item['link2'] ?>">
                    </div>
                <?php } ?>
                ...
                <?php if (isset($config['photo']['man_photo'][$type]['loaidieuhuong_photo']) && $config['photo']['man_photo'][$type]['loaidieuhuong_photo'] == true) { ?>
                    <div class="form-group d-inline-block mb-2 mr-2">
                        ...
                        <div class="custom-control custom-checkbox d-inline-block align-middle">
                            <input type="checkbox" class="custom-control-input loaidieuhuong-checkbox" name="data[loaidieuhuong]" <?=($item['loaidieuhuong']==1)?'checked':''?> id="loaidieuhuong-checkbox" value="1">
                                    ...
                                    <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                        ...
                                        <?php if (isset($config['photo']['man_photo'][$type]['desc_photo']) && $config['photo']['man_photo'][$type]['desc_photo'] == true) { ?>
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control text-sm <?= (isset($config['photo']['man_photo'][$type]['desc_cke_photo']) && $config['photo']['man_photo'][$type]['desc_cke_photo'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[desc<?= $k ?>]" id="desc<?= $k ?>" rows="5" placeholder="<?=mota?> (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('desc' . $k)) ?: $func->decodeHtmlChars(@$item['desc' . $k]) ?></textarea>
                                            </div>
                                        <?php } ?>
                                        <?php if (isset($config['photo']['man_photo'][$type]['content_photo']) && $config['photo']['man_photo'][$type]['content_photo'] == true) { ?>
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control text-sm <?= (isset($config['photo']['man_photo'][$type]['content_cke_photo']) && $config['photo']['man_photo'][$type]['content_cke_photo'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[content<?= $k ?>]" id="content<?= $k ?>" rows="5" placeholder="<?=noidung?> (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('content' . $k)) ?: $func->decodeHtmlChars(@$item['content' . $k]) ?></textarea>
                                            </div>
                                        <?php } ?>
                                    </div>
                                <?php } ?>
