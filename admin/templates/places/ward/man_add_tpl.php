    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active"><?=chitietphuongxa?></li>
            </ol>
        </div>
    </div>
</section>
    ...
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <div class="card card-primary card-outline text-sm">
            <div class="card-header">
                <h3 class="card-title"><?= ($act == "edit_ward") ? capnhat : themmoi; ?></h3>
            </div>
            <div class="card-body">
                <div class="form-group-category row">
                    <div class="form-group col-md-3 col-sm-4">
                        <label class="d-block" for="id_city"><?=danhsachtinhthanh?>:</label>
                        <?= $func->getAjaxPlace("city") ?>
                    </div>
                    <div class="form-group col-md-3 col-sm-4">
                        <label class="d-block" for="id_district"><?=danhsachquanhuyen?>:</label>
                        <?= $func->getAjaxPlace("district") ?>
                    </div>
                </div>
                <div class="form-group">
                    <label for="name"><?=tieude?>: <span class="text-danger">*</span></label>
                ...
                <?php if (isset($config['places']['placesship']) && $config['places']['placesship'] == true) { ?>
                    <div class="form-group">
                        <label for="ship_price" class="d-block"><?=phivanchuyen?>:</label>
                        <input type="text" class="form-control format-price w-auto d-inline-block align-middle text-sm" name="data[ship_price]" id="ship_price" placeholder="<?=phivanchuyen?>" value="<?= (!empty($flash->has('ship_price'))) ? $flash->get('ship_price') : @$item['ship_price'] ?>" required>
