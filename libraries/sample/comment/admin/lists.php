...
<div class="comment-item">
    ...
    <div class="comment-item-information">
        <div class="comment-item-rating mb-2 w-clear">
            ...
            <div class="comment-item-title"><?= $this->func->decodeHtmlChars($params['lists']['title']) ?><?= (strpos($params['lists']['status'], 'new-admin') !== false) ? '<strong class="comment-new bg-danger rounded text-white small ml-2 px-2 py-1">'.moi.'</strong>' : '' ?></div>
        </div>
        ...
        <div class="comment-action mb-2">
            <a class="btn btn-sm btn-info btn-reply-comment mr-1" href="javascript:void(0)" data-name="<?= $params['lists']['fullname'] ?>"><?=traloi?></a>
            <a class="btn btn-sm <?= (strpos($params['lists']['status'], 'hienthi') !== false) ? 'btn-warning' : 'btn-primary' ?> btn-status-comment mr-1" href="javascript:void(0)" data-id="<?= $params['lists']['id'] ?>" data-new-sibling="comment-item-rating" data-status="hienthi"><?= (strpos($params['lists']['status'], 'hienthi') !== false) ? boduyet : duyet ?></a>
            <a class="btn btn-sm btn-danger btn-delete-comment" href="javascript:void(0)" data-id="<?= $params['lists']['id'] ?>" data-class="comment-item" data-parents="comment-lists"><?=xoa?></a>
        </div>
            ...
            <div class="comment-replies mt-3">
                ...
                <?php if ($this->totalReplies($params['lists']['id'], $params['id_variant'], $params['type'], $params['is_admin']) > $this->limitChildShow) { ?>
                    <div class="comment-load-more-control border-top text-left mt-4 pt-3">
                        ...
                        <button class="btn-load-more-comment-child text-primary text-decoration-none" href="javascript:void(0)" title="<?=xemthembinhluan?>"><?=xemthembinhluan?></button>
                    </div>
                <?php } ?>
            </div>
        <?php } ?>
