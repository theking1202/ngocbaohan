...
$footer = $cache->get("select name$lang, content$lang,desc$lang, photo from #_static where type = ? limit 0,1", array('footer'), 'fetch', 7200);
$chinhsach = $cache->get("select name$lang, slugvi, slugen, desc$lang, date_created, id, photo,numb from #_news where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('chinh-sach'), 'result', 7200);
