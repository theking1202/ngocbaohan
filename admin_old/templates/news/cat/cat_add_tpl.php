    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active"><?= $config['news'][$type]['title_main_cat'] ?></li>
            </ol>
        </div>
    </div>
</section>
        ...
        <div class="row">
                                        ...
                                        <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if (isset($config['news'][$type]['desc_cat']) && $config['news'][$type]['desc_cat'] == true) { ?>
                                                <div class="form-group">
                                                    ...
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['news'][$type]['desc_cke_cat']) && $config['news'][$type]['desc_cke_cat'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[desc<?= $k ?>]" id="desc<?= $k ?>" rows="5" placeholder="<?=mota?> (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('desc' . $k)) ?: $func->decodeHtmlChars(@$item['desc' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
                                            <?php if (isset($config['news'][$type]['content_cat']) && $config['news'][$type]['content_cat'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="content<?= $k ?>"><?=noidung?> (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['news'][$type]['content_cke_cat']) && $config['news'][$type]['content_cke_cat'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[content<?= $k ?>]" id="content<?= $k ?>" rows="5" placeholder="<?=noidung?> (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('content' . $k)) ?: $func->decodeHtmlChars(@$item['content' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
            ...
            <div class="col-xl-4">
                ...
                <?php if (isset($config['news'][$type]['images_cat']) && $config['news'][$type]['images_cat'] == true) { ?>
                    <div class="card card-primary card-outline text-sm">
                        <div class="card-header">
                            <h3 class="card-title"><?=hinhanh?> <?= $config['news'][$type]['title_main_cat'] ?></h3>
                            <div class="card-tools">
                                <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
                            </div>
                        </div>
