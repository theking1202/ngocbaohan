...
if (!isset($config['onesignal']) || $config['onesignal'] == false) $func->transfer(trangkhongtontai, "index.php", false);
...
function editMan()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "index.php?com=pushOnesignal&act=man&p=" . $curPage, false);
    } else {
        $item = $d->rawQueryOne("select * from #_pushonesignal where id = ? limit 0,1", array($id));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "index.php?com=pushOnesignal&act=man&p=" . $curPage, false);
        }
    }
}
...
function saveMan()
{
    ...
    if (empty($data['description'])) {
        $response['messages'][] = motakhongduoctrong;
    }
    ...
    if ($id) {
        ...
        if ($d->update('pushonesignal', $data)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "index.php?com=pushOnesignal&act=man&p=" . $curPage);
        } else {
            $func->transfer(capnhatdulieubiloi, "index.php?com=pushOnesignal&act=man&p=" . $curPage, false);
        }
    } else {
        if ($d->insert('pushonesignal', $data)) {
            ...
            $func->transfer(luudulieuthanhcong, "index.php?com=pushOnesignal&act=man&p=" . $curPage);
        } else {
            $func->transfer(luudulieubiloi, "index.php?com=pushOnesignal&act=man&p=" . $curPage, false);
        }
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "index.php?com=pushOnesignal&act=man&p=" . $curPage);
        } else {
            $func->transfer(xoadulieubiloi, "index.php?com=pushOnesignal&act=man&p=" . $curPage, false);
        }
...
{
    global $d, $langadmin, $configBase, $config;
...
function sendSync()
{
    global $d, $langadmin, $func, $curPage, $config;
    ...
    if ($id) {
        ...
        $func->transfer(guithongbaothanhcong, "index.php?com=pushOnesignal&act=man&p=" . $curPage);
    } else {
        $func->transfer(khongnhanduocdulieu, "index.php?com=pushOnesignal&act=man&p=" . $curPage, false);
    }
}
