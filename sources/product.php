...
if ($id != '') {
    ...
    $rowDetail = $d->rawQueryOne("select type, id, name$lang, slugvi, slugen, desc$lang, content$lang, code, view, id_brand, id_list, id_cat, id_item, id_sub, photo, icon, options, discount, sale_price, regular_price from #_product where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($id, $type));
    ...
    $sql = "select photo, name$lang,desc$lang, slugvi, slugen, sale_price, regular_price, discount,icon, id from #_product where $where order by numb,id desc $limit";
    ...
    $Producthot = $d->rawQuery("select photo, name$lang,desc$lang, slugvi, slugen, sale_price, regular_price, discount,icon, id from #_product where type = ? and find_in_set('hot',status) and find_in_set('hienthi',status) order by numb,id desc", array($type));
