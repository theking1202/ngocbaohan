...
function wishList()
{
    global $d, $func, $loginMember, $configBase, $lang;
    ...
    $rowDetail = $d->rawQuery("select w.*, p.id, p.namevi, p.nameen, p.photo, p.regular_price, p.sale_price, p.discount, p.slugvi, p.slugen 
                              from #_wishlist w 
                              left join #_product p on w.id_product = p.id 
                              where w.id_user = ? AND find_in_set('hienthi',p.status)
                              order by w.date_created desc", 
                              array($_SESSION[$loginMember]['id']));
